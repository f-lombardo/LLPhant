...
class LmStudioChat implements ChatInterface
{
  ...
  public function __construct(protected LmStudioConfig $config, private readonly LoggerInterface|NullLogger $logger = new NullLogger())
  {
  ...
  public function generateChatOrReturnFunctionCalled(array $messages): array|string
  {
    ...
    // Only add tools if we have any (don't send empty array)
    if (!empty($this->tools)) {
    ...
    // Check if tool_calls exists AND is not empty before accessing [0]
